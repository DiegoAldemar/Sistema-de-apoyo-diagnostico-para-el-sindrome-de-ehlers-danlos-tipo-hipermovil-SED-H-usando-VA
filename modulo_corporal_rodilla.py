...
from modulo_corporal_pies import *


class Ventana_corporal_rodilla(QMainWindow, QThread):
    changePixmap = pyqtSignal(QImage)
    def __init__(self, parent=None):
        super(Ventana_corporal_rodilla, self).__init__(parent)
        loadUi('./archivos_ui/VENTANA_CORPORAL_RODILLA.ui', self)
        self.cont = 0
        self.Nfoto = 0
        self.inicio = 0 
        self.culito = 1
        ...
        movie = QMovie('./imagenes/rodillagif.gif')
        self.gif_rodilla.setMovie(movie)
        movie.start()
    ...
    def siguiente_ventana(self):
        try:
            self.cap.release()
            self.hide()
            otraventana = Ventana_corporal_pies(self)
    ...
    def start_camara(self):
        ...
        if(os.path.isfile('./fotos_cuerpo/foto_rodilla_1.png')):
            self.Listo.setText('Foto 1/2 (Falta)')
            self.Toma.setText('1) Toma de Rodilla Izquierda')
            self.b_Capturar.setEnabled(True)
        else:
           self.Listo.setText('Foto 0/2 (Falta)')
           self.Toma.setText('1) Toma de Rodilla Derecha')

        if(os.path.isfile('./fotos_cuerpo/foto_rodilla_2.png')):
        ...
        try:
            ...
            if(self.Nfoto == 2):
                ...
                cv2.imwrite('./fotos_cuerpo/foto_rodilla_%s.png'%(self.cont), rotated)
                self.Nfoto = 1
    ...
    def ver_foto(self):
        try:
            ...
            foto_cuerpo = cv2.imread('./fotos_cuerpo/foto_rodilla_%s.png'%(self.cont), 1) 
    ...
    def borrar_foto(self):
        try:
            cv2.destroyAllWindows()
            os.remove('./fotos_cuerpo/foto_rodilla_%s.png'%(self.cont))  #nota importante evitar los errores cuando se trabaj python
            self.cont = self.cont - 1
    ...
    def volver(self):
        try:
           self.cap.release()
           os.remove('./fotos_cuerpo/foto_rodilla_1.png')  #nota importante evitar los errores cuando se trabaj python
           os.remove('./fotos_cuerpo/foto_rodilla_2.png') 
